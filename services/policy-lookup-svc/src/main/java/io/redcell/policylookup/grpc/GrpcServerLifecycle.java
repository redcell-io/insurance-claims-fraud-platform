...
public class GrpcServerLifecycle implements SmartLifecycle {
    ...
    private final PolicyLookupGrpcService policyLookupGrpcService;
    ...
    public GrpcServerLifecycle(
            PolicyLookupGrpcService policyLookupGrpcService,
            @Value("${app.grpc.port:9096}") int port) {
        this.policyLookupGrpcService = policyLookupGrpcService;
        this.port = port;
    }
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(port)
                    .addService(policyLookupGrpcService)
                    .build()
                    .start();
            running = true;
            log.info("policy-lookup-svc gRPC server listening on :{}", port);
        } catch (IOException e) {
